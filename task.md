# Task List - 7th Sea Character Sheet App

## Current Session Tasks

- [/] Planning 7th Sea Character App <!-- id: 80 -->
    - [x] Create implementation plan <!-- id: 81 -->
    - [x] Wait for PDF reference <!-- id: 82 -->
    - [x] Analyze PDF for data structure <!-- id: 83 -->

- [x] Project Setup & Scaffolding <!-- id: 84 -->
    - [x] Initialize project structure <!-- id: 85 -->
    - [x] Create PWA manifest and service worker <!-- id: 86 -->
    - [x] Create parchment/nautical theme CSS <!-- id: 87 -->

- [x] Refactor for Dual Edition Support <!-- id: 112 -->
    - [x] Restructure data folders (v1/v2) <!-- id: 113 -->
    - [x] Update CreateWizard for Edition Selection <!-- id: 114 -->
    - [x] Implement V1 Data (Nations, Skills, Schools) <!-- id: 115 -->
    - [x] Update DiceRoller for Roll & Keep (V1) <!-- id: 116 -->

- [/] Core Data Model <!-- id: 88 -->
    - [x] Define character schema (Traits, Skills, Advantages) <!-- id: 89 -->
    - [x] Implement local storage persistence <!-- id: 90 -->
    - [x] Multi-character support <!-- id: 91 -->

- [/] Character Creation Wizard <!-- id: 92 -->
    - [x] Step 0: Edition Selection <!-- id: 117 -->
    - [x] Step 1: Identity & Nation (Image, Details) <!-- id: 93 -->
    - [x] Step 2: Traits allocation <!-- id: 94 -->
    - [x] Step 3: Background selection <!-- id: 95 -->
    - [x] Step 4: Skills selection <!-- id: 96 -->
    - [x] Step 5: Advantages selection <!-- id: 97 -->
    - [x] Step 6: Stories & Arcana <!-- id: 98 -->

- [x] Character Sheet View <!-- id: 99 -->
    - [x] Main sheet layout (parchment style) <!-- id: 100 -->
    - [x] Traits & Skills display <!-- id: 101 -->
    - [x] Advantages section <!-- id: 102 -->
    - [x] Wounds & Drama dice tracking <!-- id: 103 -->

- [x] Gameplay Features <!-- id: 104 -->
    - [x] D10 dice roller with animations <!-- id: 105 -->
    - [x] Raise calculator <!-- id: 106 -->
    - [x] Hero Points tracker <!-- id: 107 -->

- [/] Deployment <!-- id: 108 -->
    - [x] Create GitHub repository <!-- id: 109 -->
    - [x] Configure GitHub Pages <!-- id: 110 -->
    - [ ] Test PWA on iPhone <!-- id: 111 -->

- [/] UI Redesign <!-- id: 120 -->
    - [x] Liquid Glass Navigation <!-- id: 121 -->
    - [x] Modern Character List (Search, Cards, FAB) <!-- id: 122 -->

- [ ] Character Growth & Management (v0.4) <!-- id: 130 -->
    - [ ] Data Model: Inventory & Journal <!-- id: 131 -->
    - [ ] UI: Sheet Tabs (Stats, Equip, Journal) <!-- id: 132 -->
    - [ ] UI: Editable Wealth & XP <!-- id: 133 -->
    - [ ] Logic: Level Up / Advancement Mode <!-- id: 134 -->

- [ ] Interactive Character Sheet (v0.5) <!-- id: 150 -->
    - [ ] Tooltip System (Descriptions on tap) <!-- id: 151 -->
- [x] List Enhancements (v0.6) <!-- id: 160 -->
    - [x] Swipe-to-Edit / Swipe-to-Delete <!-- id: 161 -->
    - [x] "Edit Profile" Modal (Restricted fields) <!-- id: 162 -->
- [x] Sheet Refinements (v0.6) <!-- id: 170 -->
    - [x] Standardize Layout (Advantages/Backgrounds) <!-- id: 171 -->
    - [x] Clean up Translations <!-- id: 172 -->
    - [x] Move "Level Up" UI <!-- id: 173 -->
- [ ] Logic: XP Spending & Rank Cost <!-- id: 180 -->

## Completed Tasks

- [x] Seiryuu Dojo App (previous project) <!-- id: 0 -->
